...
- (void)runStateMachineAt:(int)newState
{
    ...
    switch (newState)
    {
        ...
        default:
            // We are presenting a UIAlertController here and, once this time delay expires,
            // it is being dismissed stranding the user with no options. We don't want this
            // behavior so for now we are getting rid of this delay.
...
{
    NSBundle* bundle = [NSBundle bundleForClass:MiSnapSDKViewController.self];
...
- (void)cancelMiSnap
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (wself.helpViewController != nil)
        {
            ...
            // NOTE: The below lines cause a crash in the D3 App
            // if (wself.navigationController == nil)
            // {
...
- (void)showSmartTutorialWithNumberOfButtons:(int)numberOfButtons forDocumentType:(NSString *)documentType forTutorialMode:(MiSnapTutorialMode)tutorialMode
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        @try {
            ...
            wself.helpViewController = [[UIStoryboard storyboardWithName:@"MiSnapUX2" bundle:[NSBundle bundleForClass:MiSnapSDKViewController.self]] instantiateViewControllerWithIdentifier:@"MiSnapSDKTutorialViewController"];
        } @catch (NSException *exception) {
            wself.helpViewController = [[UIStoryboard storyboardWithName:@"MiSnapUX1" bundle:[NSBundle bundleForClass:MiSnapSDKViewController.self]] instantiateViewControllerWithIdentifier:@"MiSnapSDKTutorialViewController"];
        }
        ...
        if ([self.timeoutResults count] > 0) {
            ...
            wself.helpViewController.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"background-dots" inBundle:[NSBundle bundleForClass:MiSnapSDKViewController.self] compatibleWithTraitCollection:nil]];
            ...
            for (NSString *hint in self.timeoutResults) {
                ...
                UIImageView *hintPointerView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"misnap_tutorial_red_arrow" inBundle:[NSBundle bundleForClass:MiSnapSDKViewController.self] compatibleWithTraitCollection:nil]];
